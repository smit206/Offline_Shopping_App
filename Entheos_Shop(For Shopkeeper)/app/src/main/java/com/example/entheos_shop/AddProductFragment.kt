...
class AddProductFragment : Fragment() {
            ...
            fileRef.putFile(photoUri).addOnSuccessListener {
                fileRef.downloadUrl.addOnSuccessListener { uri ->
                    ...
                    if(photoUrls.size == selectedPhotos.size){
                        saveProductToDatabase(productName,price,description,photoUrls)
                    }
                }
    ...
    @SuppressLint("NotifyDataSetChanged")
    private fun saveProductToDatabase(name: String, price: String, description: String, photoUrls: List<String>){
        val currentUserId = FirebaseAuth.getInstance().currentUser?.uid
        if(currentUserId != null){
            val productId = databaseReference.push().key
            val product = Product(name, price, description, photoUrls,currentUserId)
            if(productId != null){
                databaseReference.child(productId).setValue(product).addOnCompleteListener { task ->
                    if (task.isSuccessful){
                        ...
                        Toast.makeText(context, "Failes to upload product", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        }
            ...
            databaseReference.orderByChild("userId").equalTo(currentUserId)
                .addListenerForSingleValueEvent(object: ValueEventListener{
                    ...
                    override fun onCancelled(error: DatabaseError) {
                        Toast.makeText(context, "Failed to load user products", Toast.LENGTH_SHORT).show()
                    }
